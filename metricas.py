"""Métricas del motor sobre data/motor.db -- compartido entre dashboard.py y app.py.

No genera datos -- motor.registrar() es quien escribe en la DB cada vez que
recomendar() resuelve un perfil. Este módulo solo lee y renderiza. Estructura
de tablas: ver scripts/crear_db.py. Campos de recomendar(): ver motor.py.

render_metricas() se puede llamar desde cualquier página de Streamlit que ya
haya hecho su propio st.set_page_config() -- este módulo no lo llama, para no
chocar con el de quien lo importe (dashboard.py standalone, o la pestaña de
métricas de app.py).
"""
import json
import sqlite3
...
def _activar_autorefresco():
    try:
        ...
        st_autorefresh(interval=10000, limit=None, key="metricas_autorefresh")
    except ImportError:
...
def render_metricas():
    """Dibuja las 7 secciones de métricas en la página actual. No hace st.stop() --
    usa return, para no cortar el resto de la página si se llama desde una pestaña."""
    _activar_autorefresco()

    if st.button("Refrescar ahora", key="metricas_refrescar"):
        cargar_datos.clear()
    ...
    fig_productos.update_traces(marker_color=AZUL)
    ...
    fig_canal.update_traces(marker_color=AMARILLO)
